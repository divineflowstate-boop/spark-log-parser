...
public class SparkEventLogParser {
  ...
  public CompactParsedLog parse(Path eventLog) throws Exception {
    ...
    try (BufferedReader br = open(eventLog)) {
      ...
      while ((line = br.readLine()) != null) {
        ...
        switch (ev.asText()) {
          ...
          default -> {
            // SQL is best-effort; ignore for MVP unless you want to add later
          }
        }
      }
    }
    ...
    // Print debug stats so you can confirm parsing quickly
  ...
  /**
   * FIXED for Spark 3.5.x event logs:
   * TaskEnd typically has Stage ID / Stage Attempt ID at TOP LEVEL, not inside "Stage Info".
   */
  private void onTaskEnd(JsonNode e) {
    seenTaskEnd++;

    // Preferred: top-level fields (most common)
    Integer stageId = intAtOrNull(e, "Stage ID");
    Integer attemptId = intAtOrNull(e, "Stage Attempt ID");

    // Fallback: Stage Info if present in your environment
  ...
  private void finalizeExecutorsAvg() {
    ...
    execCountTimeline.putIfAbsent(start, execCountTimeline.floorEntry(start) != null ? execCountTimeline.floorEntry(start).getValue() : 0);
